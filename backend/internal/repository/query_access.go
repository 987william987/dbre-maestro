...
func (r *QueryAccessRepo) ListAllActiveRulesForSubjects(ctx context.Context, subjectID uint64, authGroupIDs []uint64) ([]model.QueryAccessRule, error) {
	args := []any{subjectID}
	authGroupClause := ""
	if len(authGroupIDs) > 0 {
		query, inArgs, err := sqlx.In(` OR (qar.subject_type = 'auth_group' AND qar.subject_id IN (?))`, authGroupIDs)
	...
	args = append(args, timeutil.NowUTC())
	...
	query := `
		...
		ORDER BY
		  CASE WHEN qar.expires_at IS NULL THEN 1 ELSE 0 END,
		  qar.expires_at ASC,
		  qar.connection_id ASC,
		  qar.id DESC
	`
	rules := []model.QueryAccessRule{}
	if err := r.db.SelectContext(ctx, &rules, r.db.Rebind(query), args...); err != nil {
		return nil, fmt.Errorf("list all active query access rules for subjects: %w", err)
	}
	return rules, nil
}
